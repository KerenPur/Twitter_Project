...
add_tweet = ("INSERT INTO tweets "
             "(user_nickname, num_replies, num_likes, num_retweets) "
             "VALUES (%s, %s, %s, %s)")
...
def store_tweets_dict(tweets_dict, search, user):
    ...
    for tweet in tweets_dict:
        cursor.execute(add_tweet, [tweets_dict[tweet].user, tweets_dict[tweet].replies, tweets_dict[tweet].likes,
                                   tweets_dict[tweet].retweets])
        # cnx.commit()
        tweet_ind = cursor.lastrowid
        cursor.execute(add_search_tweets, [tweet_ind, search_id])
        # cnx.commit()
        print("Tweet from {} added successfully.".format(tweet))
        cursor.execute(add_tweets_username_searches, [tweet_ind, username_search_ind])
        for hashtag in tweets_dict[tweet].hashtags:
            ...
            cursor.execute(add_tweet_hashtags, [hashtag_ind, tweet_ind])
